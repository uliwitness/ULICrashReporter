...
unsigned	ULIPhysicalRAMSize(void)
{
	return (unsigned) (([NSProcessInfo.processInfo physicalMemory] / 1024ULL) / 1024ULL);
}


NSString*	ULISystemVersionString(void)
{
	static NSString*	sSysVersionCocoaStr = nil;
	if( !sSysVersionCocoaStr )
	{
		sSysVersionCocoaStr = [[NSDictionary dictionaryWithContentsOfFile:@"/System/Library/CoreServices/SystemVersion.plist"] objectForKey:@"ProductVersion"];
	}
	return sSysVersionCocoaStr;
}


void	ULIGetSystemVersionComponents( SInt32* outMajor, SInt32* outMinor, SInt32* outBugfix )
{
	NSArray		*	sysVersionComponents = [ULISystemVersionString() componentsSeparatedByString: @"."];
...
NSString*	ULIMachineName(void)
{
	...
	static NSDictionary* sTranslationDictionary = nil;
	if( !sTranslationDictionary )
		sTranslationDictionary = [[NSDictionary alloc] initWithObjectsAndKeys:
	...
	NSEnumerator	*e = [[[sTranslationDictionary allKeys]
	...
	while( aKey = [e nextObject] )
	{
		...
		if( r.location != NSNotFound )
		{
			...
			{
				humanReadableName = [sTranslationDictionary objectForKey:aKey];
				foundKey = aKey;
			}
		}
	}
	...
	if( humanReadableName == nil )
		sCPUName = [NSString stringWithFormat: @"Unknown (%@)", internalName];
	else
		sCPUName = humanReadableName;
	
	return sCPUName;
}


NSString*	ULICPUName(void)
{
	static NSString	*	sCPUName = nil;
